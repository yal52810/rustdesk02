...
func (us *UserService) CheckUserEnable(u *model.User) bool {
	...
	// Admins are exempt from expiration checks
	if u.IsAdmin != nil && *u.IsAdmin {
		return true
	}

	// Lifetime accounts (valid_days == -1) never expire
	if u.ValidDays == -1 {
		return true
	}
	...
	// Check expiration
	expirationDate := u.FirstLoginAt.AddDate(0, 0, u.ValidDays)
...
// SetFirstLoginIfNeeded sets first_login_at if it's nil (first login)
func (us *UserService) SetFirstLoginIfNeeded(u *model.User) {
	if u.FirstLoginAt == nil {
		now := time.Now()
		u.FirstLoginAt = &now
		DB.Model(u).Update("first_login_at", now)
	}
}
