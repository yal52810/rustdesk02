...
func ApiInit(g *gin.Engine) {
	...
	{
		p := &api.Password{}
		frg.POST("/password/send-reset-code", p.SendResetCode)
		frg.POST("/password/reset", p.ResetByCode)
	}
	...
	{
		ds := &api.DeviceSessionController{}
		frg.POST("/device/connect", ds.Connect)
		frg.POST("/device/heartbeat", ds.Heartbeat)
		frg.POST("/device/disconnect", ds.Disconnect)
		frg.GET("/device/sessions", ds.GetSessions)
	}
	{
		gr := &api.Group{}
