...
func UserBind(rg *gin.RouterGroup) {
	...
	aRP := rg.Group("/user").Use(middleware.AdminPrivilege())
	{
		...
		aRP.POST("/quickBatchCreate", cont.QuickBatchCreate)
...
func ActivationCodeBind(rg *gin.RouterGroup) {
	aR := rg.Group("/activation_code").Use(middleware.AdminPrivilege())
	{
		cont := &admin.ActivationCodeController{}
		aR.GET("/list", cont.List)
		aR.POST("/create", cont.Create)
		aR.POST("/batchCreate", cont.BatchCreate)
		aR.POST("/delete/:id", cont.Delete)
	}
}

func PackageBind(rg *gin.RouterGroup) {
	aR := rg.Group("/package").Use(middleware.AdminPrivilege())
	{
		cont := &admin.Package{}
		aR.GET("/list", cont.List)
		aR.GET("/detail/:id", cont.Detail)
...
func ServerBind(rg *gin.RouterGroup) {
	aR := rg.Group("/server").Use(middleware.AdminPrivilege())
	{
		cont := &admin.Server{}
		aR.GET("/list", cont.List)
		aR.GET("/detail/:id", cont.Detail)
