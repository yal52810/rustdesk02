...
func (o *Oauth) OidcAuthQueryPre(c *gin.Context) (*model.User, *model.UserToken) {
	...
	// Set first_login_at on first successful OAuth login
	service.AllService.UserService.SetFirstLoginIfNeeded(u)
...
func (o *Oauth) OauthCallback(c *gin.Context) {
	...
	if action == service.OauthActionTypeBind {
		...
		// Set first_login_at on first successful OAuth login
		service.AllService.UserService.SetFirstLoginIfNeeded(user)
