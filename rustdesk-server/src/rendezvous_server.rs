...
type WssSink = SplitSink<tokio_tungstenite::WebSocketStream<tokio_rustls::server::TlsStream<TcpStream>>, tungstenite::Message>;
...
impl RendezvousServer {
    ...
    pub async fn start(port: i32, serial: i32, key: &str, rmem: usize) -> ResultType<()> {
        ...
        let tls_cert = get_arg("tls-cert");
        let tls_key = get_arg("tls-key");
        let tls_acceptor = Self::load_tls_config(&tls_cert, &tls_key)?;

        if tls_acceptor.is_some() {
            log::info!("TLS enabled for WebSocket on port :{}", ws_port);
            log::info!("Listening on secure websocket (WSS) :{}", ws_port);
        } else {
                        ...
                        Some(key) => {
                            if let Some(sink) = sink.as_mut() {
                                match sink {
                                    ...
                                    Sink::WssTls(s) => s.encrypt = Some(Encrypt::new(key)),
    ...
    async fn handle_punch_hole_request(
        ...
        if MUST_LOGIN.load(Ordering::SeqCst) {
            if ph.token.is_empty() {
                let mut msg_out = RendezvousMessage::new();
                msg_out.set_punch_hole_response(PunchHoleResponse {
                    other_failure: String::from("连接失败，请登录！"),
                    ..Default::default()
                });
                return Ok((msg_out, None));
            } else if !jwt::SECRET.is_empty() {
                let token = ph.token;
               let token_result = jwt::verify_token(token.as_str());
                
                if token_result.is_err() {
                    let mut msg_out = RendezvousMessage::new();
                    msg_out.set_punch_hole_response(PunchHoleResponse {
                        other_failure: String::from("令牌错误，请注销并重新登录！"),
                        ..Default::default()
                    });
                    return Ok((msg_out, None));
                } else {
                    // SaaS logic
                    let claims = token_result.unwrap();
                    let user_id = claims.user_id;
                    let target_device_id = &ph.id;
                    let current_device_id = &claims.uuid; // (发起方设备 ID)

                    // [SaaS 1: 检查封禁]
                    match self.pm.db.get_user_status(user_id).await {
                        Ok(user) => {
                            if user.status == "banned" {
                                let mut msg_out = RendezvousMessage::new();
                                msg_out.set_punch_hole_response(PunchHoleResponse {
                                    other_failure: String::from("该帐户已被禁止"),
                                    ..Default::default()
                                });
                                return Ok((msg_out, None));
                            }
                        ...
                        Err(e) => {
                            log::error!("数据库错误 (get_user_status): {}", e);
                            let mut msg_out = RendezvousMessage::new();
                            msg_out.set_punch_hole_response(PunchHoleResponse {
                                other_failure: String::from("服务器内部错误 (SaaS-U)"),
                                ..Default::default()
                            });
                            return Ok((msg_out, None));
                        }
                    ...
                    // [SaaS 2: 检查订阅和并发限制]
                    let subscription = match self.pm.db.get_subscription_status(user_id).await {
                        Ok(Some(sub)) => {
                            if sub.expiration_datetime <= Utc::now().naive_utc() {
                                let mut msg_out = RendezvousMessage::new();
                                msg_out.set_punch_hole_response(PunchHoleResponse {
                                    other_failure: String::from("订阅已过期"),
                                    ..Default::default()
                                });
                                return Ok((msg_out, None));
                            }
                        ...
                        Ok(None) => {
                            let mut msg_out = RendezvousMessage::new();
                            msg_out.set_punch_hole_response(PunchHoleResponse {
                                other_failure: String::from("未找到有效订阅"),
                                ..Default::default()
                            });
                            return Ok((msg_out, None));
                        }
                        Err(e) => {
                            log::error!("数据库错误 (get_subscription_status): {}", e);
                            let mut msg_out = RendezvousMessage::new();
                            msg_out.set_punch_hole_response(PunchHoleResponse {
                                other_failure: String::from("服务器内部错误 (SaaS-S)"),
                                ..Default::default()
                            });
                            return Ok((msg_out, None));
                        }
                    ...
                    // [SaaS 3: 检查并发 (Redis)]
                    let concurrent_check_ok = redis_session::check_and_register(
                        user_id,
                        current_device_id,
                       subscription.concurrent_limit as i32,
                    )
                    .await;

                    if !concurrent_check_ok {
                        let mut msg_out = RendezvousMessage::new();
                        msg_out.set_punch_hole_response(PunchHoleResponse {
                            other_failure: String::from("并发检查失败 (Redis)"),
                            ..Default::default()
                        });
                        return Ok((msg_out, None));
                    }
                    ...
                    // [SaaS 4: 检查设备权限]
                    match self.pm.db.check_device_permission(user_id, target_device_id).await {
                        Ok(has_permission) => {
                            if !has_permission {
                                let mut msg_out = RendezvousMessage::new();
                                msg_out.set_punch_hole_response(PunchHoleResponse {
                                    other_failure: String::from("拒绝访问此设备"),
                                    ..Default::default()
                                });
                                return Ok((msg_out, None));
                            }
                        ...
                        Err(e) => {
                            log::error!("数据库错误 (check_device_permission): {}", e);
                            let mut msg_out = RendezvousMessage::new();
                            msg_out.set_punch_hole_response(PunchHoleResponse {
                                other_failure: String::from("服务器内部错误 (SaaS-P)"),
                                ..Default::default()
                            });
                            return Ok((msg_out, None));
                        }
    ...
    async fn handle_listener_inner(
        ...
        if ws {
            ...
            if let Some(tls_acceptor) = &self.inner.tls_acceptor {
                // TLS WebSocket (WSS)
                ...
                sink = Some(Sink::WssTls(SafeWssSink {
                    sink: a,
                    encrypt: None,
                }));
    ...
    fn load_tls_config(cert_path: &str, key_path: &str) -> ResultType<Option<Arc<TlsAcceptor>>> {
        ...
        if keys.is_empty() {
            bail!("No private key found in key file");
        }
